...
func TestCampaignLifecycleOwnerRetentionAuditAndDelete(t *testing.T) {
	...
	usage, err := service.LifecycleUsage(owner)
	if err != nil || len(usage.Owners) != 1 || usage.Owners[0].CampaignCount != 1 ||
		...
		t.Fatalf("campaign usage=%+v err=%v", usage, err)
	}
	if _, err := service.CreateRunAs(context.Background(), other, validCreateRequest()); err != nil {
		t.Fatalf("create other owner run: %v", err)
	}
	usageAfterOtherOwnerMutation, err := service.LifecycleUsage(owner)
	if err != nil {
		t.Fatalf("campaign usage after other owner mutation: %v", err)
	}
...
func TestCampaignCollectionReleasesControlledPairInSamePlan(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("digest campaign: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("digest campaign decision: %v", err)
	}
