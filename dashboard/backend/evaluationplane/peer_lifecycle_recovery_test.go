...
func TestPeerOpenDoesNotCommitVisibleLifecyclePolicyExpansion(t *testing.T) {
	...
	err = service.store.initializeLifecyclePolicyUnlocked(expanded)
	service.store.lifecycle.mu.Unlock()
	if err != nil || service.store.lifecyclePolicy.Limits != expanded {
		t.Fatalf("owner retry did not commit lifecycle policy expansion: policy=%+v err=%v", service.store.lifecyclePolicy.Limits, err)
	}
}
