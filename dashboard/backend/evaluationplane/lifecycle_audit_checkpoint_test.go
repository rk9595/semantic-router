...
func TestStartupCleanupPreservesPostCheckpointActiveSuffix(t *testing.T) {
	...
	anchored, err := service.CreateRunAs(context.Background(), owner, validCreateRequest())
	if err != nil {
		t.Fatalf("create checkpoint anchor run: %v", err)
	}
	remnantPath, remnant := checkpointLifecycleAuditAndCaptureTail(t, service.store)
	if err := os.WriteFile(remnantPath, remnant, 0o600); err != nil {
		t.Fatalf("restore checkpoint cleanup crash remnant: %v", err)
	}
	activeRun, err := service.CreateRunAs(context.Background(), owner, validCreateRequest())
	if err != nil {
		t.Fatalf("create post-checkpoint active run: %v", err)
	}
	activeLifecycle, err := service.store.readRunLifecycle(activeRun)
	if err != nil {
		t.Fatalf("read post-checkpoint run lifecycle: %v", err)
	}
	service.store.lifecycle.mu.Lock()
	...
	if err != nil {
		t.Fatalf("restart with active checkpoint suffix: %v", err)
	}
	if _, err := os.Lstat(remnantPath); !os.IsNotExist(err) {
...
func TestLifecycleCheckpointBindingCleanupRecoversOnRestart(t *testing.T) {
	...
	service.store.lifecycle.mu.Unlock()
	if err == nil || !cleaner.failed || !pending {
		t.Fatalf("binding cleanup failure was not retained: err=%v failed=%t pending=%t", err, cleaner.failed, pending)
	}
	...
	reopened, err := newStoreWithLifecycleLimits(root, limits)
	if err != nil {
		t.Fatalf("restart did not recover pending binding cleanup: %v", err)
	}
	reopened.lifecycle.mu.Lock()
