...
func TestTerminalRunRetryClosesVisibleStatusSyncFailure(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("create terminal durability run: %v", err)
	}
	...
	durable, err := restarted.GetRunAs(SystemActor(), run.ID)
	if err != nil || durable.Status != StatusCancelled || durable.CompletedAt == nil ||
		!durable.CompletedAt.Equal(completedAt) {
		t.Fatalf("terminal status changed after restart: run=%+v err=%v", durable, err)
	}
}
...
func TestIdempotentStartClosesTerminalStatusSyncFailure(t *testing.T) {
	...
	run, err := service.CreateRunAs(context.Background(), SystemActor(), validCreateRequest())
	if err != nil {
		t.Fatalf("create idempotent start durability run: %v", err)
	}
	startedAt := time.Now().UTC().Truncate(time.Microsecond)
	completedAt := startedAt.Add(time.Second)
	...
	restarted := reopenTestService(t, root)
	durable, err := restarted.GetRunAs(SystemActor(), run.ID)
	if err != nil || durable.Status != StatusCancelled {
		t.Fatalf("idempotent start status changed after restart: run=%+v err=%v", durable, err)
	}
}

func TestWorkerProgressCannotResurrectRunCancelledByPeer(t *testing.T) {
	owner, _ := newTestService(t, &controlledProcess{}, 1)
	run, err := owner.CreateRunAs(context.Background(), SystemActor(), validCreateRequest())
	if err != nil {
		t.Fatalf("create ordinary progress race run: %v", err)
	}
	...
	stale, err := owner.store.GetRun(run.ID)
	if err != nil || stale.Status != StatusRunning {
		t.Fatalf("read stale running snapshot: run=%+v err=%v", stale, err)
	}
	cancelled, err := peer.CancelRunAs(SystemActor(), run.ID)
	if err != nil || cancelled.Status != StatusCancelled {
		t.Fatalf("peer cancel ordinary run: run=%+v err=%v", cancelled, err)
	}
	...
	durable, err := owner.store.GetRun(run.ID)
	if err != nil || durable.Status != StatusCancelled || durable.CompletedAt == nil {
		t.Fatalf("peer cancellation was not preserved: run=%+v err=%v", durable, err)
	}
}
