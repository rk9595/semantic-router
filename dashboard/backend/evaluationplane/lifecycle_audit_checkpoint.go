...
func (s *Store) checkpointLifecycleAuditUnlocked(now time.Time) error {
	...
	desiredBindings, err := s.lifecycleCreationBindingsForCheckpointUnlocked()
	if err != nil {
		return err
	}
	...
	segmentStart, segmentRoot, err := s.lifecycleCheckpointSegmentBoundaryUnlocked()
	if err != nil {
		return err
	}
	...
	if err != nil || int64(len(encoded)+1) > s.maxLifecycleCheckpointBytes() {
...
func validateCompactedLifecycleAuditSuffix(
	...
	for index, record := range records {
		if record.Sequence != first.Sequence+uint64(index) || record.PreviousDigest != previous {
