...
func TestControlledPairRunningRetryClosesAggregateSyncFailure(t *testing.T) {
	...
	visible, err := service.store.readControlledPair(pair.PairID)
	if err != nil || visible.State != controlledPairStateRunning {
		t.Fatalf("failed running publication is not retryable: state=%s err=%v", visible.State, err)
	}
	...
	service.store.lifecycle.mu.Unlock()
	if err != nil || started.Pair.State != controlledPairStateRunning {
		t.Fatalf("controlled pair retry did not close aggregate durability: state=%s err=%v", started.Pair.State, err)
	}
	...
	if err != nil {
		t.Fatalf("reopen durable controlled pair: %v", err)
	}
	durable, err := reopened.readControlledPair(pair.PairID)
	if err != nil || durable.State != controlledPairStateRunning {
		t.Fatalf("controlled pair state changed after restart: state=%s err=%v", durable.State, err)
	}
}
...
func TestControlledPairTerminalRetryClosesMemberAndAggregateSyncFailure(t *testing.T) {
	...
	service.store.lifecycle.mu.Unlock()
	if err != nil {
		t.Fatalf("start controlled pair terminal fixture: %v", err)
	}
	recorder := &recordingControlledPairPersistence{
		delegate:                    service.store.pairPersistence,
		failManifestDirectorySyncAt: 2,
	...
	visible, err := service.store.readControlledPair(pair.PairID)
	if err != nil || visible.State != controlledPairStateTerminal {
		t.Fatalf("failed terminal publication is not retryable: state=%s err=%v", visible.State, err)
	}
	...
	service.store.lifecycle.mu.Lock()
	terminal, err := service.store.cancelControlledPairAs(SystemActor(), pair.PairID)
	service.store.lifecycle.mu.Unlock()
	if err != nil || terminal.State != controlledPairStateTerminal ||
		terminal.BaselineRun.Status != StatusCancelled || terminal.CandidateRun.Status != StatusCancelled {
		t.Fatalf("controlled pair retry did not close terminal durability: pair=%+v err=%v", terminal, err)
	}
	...
	reopened, err := newStandaloneStore(root)
	if err != nil {
		t.Fatalf("reopen terminal controlled pair: %v", err)
	}
	durable, err := reopened.readControlledPair(pair.PairID)
	if err != nil || durable.State != controlledPairStateTerminal ||
		durable.BaselineRun.Status != StatusCancelled || durable.CandidateRun.Status != StatusCancelled {
		t.Fatalf("terminal controlled pair changed after restart: pair=%+v err=%v", durable, err)
	}
}
