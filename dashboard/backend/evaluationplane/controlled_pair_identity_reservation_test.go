...
func TestDeletedControlledPairPermanentlyReservesMemberRunIdentities(t *testing.T) {
	...
	tombstoneBytes, err := privateDirectoryBytes(
		filepath.Join(service.store.controlledPairRoot, pair.PairID), "",
	)
	if err != nil || tombstoneBytes == 0 {
		t.Fatalf("measure controlled pair tombstone: bytes=%d err=%v", tombstoneBytes, err)
	}
	...
	otherBaselineSource := createStoreControlledPairSource(t, service, other)
	otherCandidateSource := createStoreControlledPairSource(t, service, other)
	for role, reservedID := range map[string]string{
		"baseline": pair.BaselineRunID, "candidate": pair.CandidateRunID,
	} {
