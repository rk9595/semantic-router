...
func TestRunReadAPIsEnforceOwnerAndAdministratorBoundaries(t *testing.T) {
	...
	if err != nil || adminPage.TotalRuns != 4 || len(adminPage.Runs) != 4 {
		t.Fatalf("administrator ledger=%+v err=%v, want all four runs", adminPage, err)
	}
	...
	foreignEvents, foreignUnsubscribe, err := service.SubscribeAs(ownerB, a1.ID)
	if !errors.Is(err, ErrForbidden) || foreignEvents != nil || foreignUnsubscribe != nil {
		t.Fatalf("foreign SubscribeAs events=%v unsubscribe=%v error=%v", foreignEvents, foreignUnsubscribe, err)
	}
...
func TestRunLedgerScopesQuarantineWarningsToTheImmutableOwner(t *testing.T) {
	...
	originalLifecycle, err := service.store.readRunLifecycle(quarantined)
	if err != nil {
		t.Fatalf("read original lifecycle evidence: %v", err)
	}
	if err := os.WriteFile(
	...
	ownerPage, err := service.ListRunLedgerPageAs(ownerA, RunListQuery{Limit: 10})
	if err != nil || ownerPage.TotalRuns != 1 || len(ownerPage.Runs) != 1 || ownerPage.Runs[0].ID != healthy.ID {
		t.Fatalf("owner ledger=%+v err=%v", ownerPage, err)
	}
	...
	quarantinedOwnerPage, err := service.ListRunLedgerPageAs(ownerB, RunListQuery{Limit: 10})
	if err != nil || quarantinedOwnerPage.TotalRuns != 0 || len(quarantinedOwnerPage.Runs) != 0 ||
		...
		t.Fatalf("unknown-owner ledger=%+v err=%v", quarantinedOwnerPage, err)
	}
	adminPage, err := service.ListRunLedgerPageAs(administrator, RunListQuery{Limit: 10})
	if err != nil || len(adminPage.Warnings) != 1 || adminPage.Warnings[0].EvidenceID != quarantined.ID {
		t.Fatalf("administrator quarantine warning=%+v err=%v", adminPage.Warnings, err)
	}
	...
	adminPage, err = service.ListRunLedgerPageAs(administrator, RunListQuery{Limit: 10})
	if err != nil || len(adminPage.Warnings) != 1 || adminPage.Warnings[0].EvidenceID != quarantined.ID {
		t.Fatalf("changed-owner quarantine warning=%+v err=%v", adminPage.Warnings, err)
	}
...
func TestRunLedgerShowsUnattributableQuarantineOnlyToAdministrators(t *testing.T) {
	...
	if err != nil || adminPage.LedgerComplete || adminPage.WarningCount != 1 || len(adminPage.Warnings) != 1 ||
		adminPage.Warnings[0].EvidenceID != evidenceID {
