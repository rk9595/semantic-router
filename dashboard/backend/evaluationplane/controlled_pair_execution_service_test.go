...
func TestCreateControlledPairExecutionDefersLaunchUntilRunningManifestSyncRetry(t *testing.T) {
	...
	visible, err := service.store.readControlledPair(request.ClientRequestID)
	if err != nil || visible.State != controlledPairStateRunning {
		t.Fatalf("uncertain running manifest is not available for explicit retry: pair=%+v err=%v", visible, err)
	}
