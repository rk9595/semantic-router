...
func TestRealWorkerAuthenticatedRoutingUsesServerBrokeredCredential(t *testing.T) {
	...
	t.Setenv("TMPDIR", "/tmp")
	const routerCredentialEnv = "ROUTER_EVAL_TOKEN"
	const routerCredential = "server-owned-router-evaluation-secret"
	t.Setenv(routerCredentialEnv, routerCredential)
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/api/v1/eval":
			if request.Header.Get("Authorization") == "Bearer "+routerCredential {
	...
	service, err := NewService(Options{
		...
		RouterAPIKeyEnv: routerCredentialEnv,
	...
	manifestBytes, err := os.ReadFile(filepath.Join(root, "runs", run.ID, manifestFileName))
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Contains(manifestBytes, []byte(`"env":"`+routerCredentialEnv+`"`)) ||
		bytes.Contains(manifestBytes, []byte(routerCredential)) {
	...
	assertAuthenticatedRoutingPublication(
		t, service, root, run, routerCredential, &authenticatedRouterRequests, &diagnostics,
	)
}
