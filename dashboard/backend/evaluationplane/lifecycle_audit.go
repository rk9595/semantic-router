...
func (s *Store) appendLifecycleAuditUnlocked(
	...
	resourceID, denialKey, deduplicated, err := s.prepareLifecycleNotFoundDenialUnlocked(
	...
	record, err := s.newLifecycleAuditRecord(
	...
	encoded, err := encodeLifecycleAuditRecord(record)
	if err != nil {
		return lifecycleAuditRecord{}, err
	}
