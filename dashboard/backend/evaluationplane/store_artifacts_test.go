...
func TestVerifiedArtifactDescriptorSurvivesConcurrentDeletion(t *testing.T) {
	service, root := newTestService(t, &controlledProcess{}, 1)
	run, _ := prepareOpenableArtifact(t, service, root)
	opened, err := service.OpenArtifactAs(SystemActor(), run.ID, "metrics")
	if err != nil {
	...
	if err != nil || string(data) != "{\"routing.accuracy\":1}\n" {
		t.Fatalf("pinned artifact after deletion data=%q err=%v", data, err)
	}
}
...
func TestVerifiedArtifactDescriptorDoesNotBlockServiceClose(t *testing.T) {
	...
	data, err := io.ReadAll(opened.File)
	if err != nil || string(data) != "{\"routing.accuracy\":1}\n" {
		t.Fatalf("pinned artifact after Service close data=%q err=%v", data, err)
	}
}
