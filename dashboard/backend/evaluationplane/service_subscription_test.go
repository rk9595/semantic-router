...
func TestSubscribersAreSharedAcrossServicesForDeleteAndBroadcast(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("create shared subscriber run: %v", err)
	}
	peer := newSubscriberPeerService(t, owner)
	t.Cleanup(func() { _ = peer.Close() })

	peerEvents, _, err := peer.SubscribeAs(SystemActor(), run.ID)
	if err != nil {
		t.Fatalf("subscribe through peer Service: %v", err)
	}
	...
	case event, open := <-peerEvents:
		if !open || event.RunID != run.ID || event.Message != "cross-service progress" {
	...
	ownerEvents, _, err := owner.SubscribeAs(SystemActor(), run.ID)
	if err != nil {
		t.Fatalf("subscribe through owner Service: %v", err)
	}
