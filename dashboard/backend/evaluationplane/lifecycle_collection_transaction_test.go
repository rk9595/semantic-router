...
func TestLifecycleCollectionTransactionResumesFrozenPlanAndReturnsReceipt(t *testing.T) {
	...
	actor := SystemActor()
	dryRun, err := service.CollectLifecycle(actor, CollectionRequest{})
	if err != nil || len(dryRun.Plan.Candidates) != 2 {
		t.Fatalf("build collection transaction plan: candidates=%d err=%v", len(dryRun.Plan.Candidates), err)
	}
	...
	other, err := NewActor("collection-transaction-other-admin", true)
	if err != nil {
		t.Fatal(err)
	}
	...
	completed, err := service.CollectLifecycle(actor, request)
	if err != nil {
		t.Fatalf("resume collection transaction: %v", err)
	}
	if !completed.Applied || len(completed.DeletedRunIDs) != 2 {
		t.Fatalf("completed collection receipt=%+v", completed)
	}
	retried, err := service.CollectLifecycle(actor, request)
	if err != nil || !reflect.DeepEqual(retried, completed) {
		t.Fatalf("completed receipt retry=%+v err=%v, want %+v", retried, err, completed)
	}
}
...
func TestLifecycleCollectionStartupRecoversPendingTransactionAndPeerFailsClosed(t *testing.T) {
	...
	dryRun, err := service.CollectLifecycle(actor, CollectionRequest{})
	if err != nil {
		t.Fatalf("build restart collection plan: %v", err)
	}
	...
	if !errors.Is(err, ErrConflict) {
		t.Fatalf("peer opener advanced pending collection transaction: %v", err)
	}
	if err := service.Close(); err != nil {
		t.Fatalf("close collection transaction owner: %v", err)
	}
	restarted := reopenTestService(t, root)
	receipt, err := restarted.CollectLifecycle(actor, request)
	if err != nil || len(receipt.DeletedRunIDs) != 2 {
		t.Fatalf("startup collection recovery receipt=%+v err=%v", receipt, err)
	}
...
func TestLifecycleCollectionVisibleTransactionWriteRequiresRetrySync(t *testing.T) {
	...
	dryRun, err := service.CollectLifecycle(SystemActor(), CollectionRequest{})
	if err != nil {
		t.Fatal(err)
	}
	faults := &faultingLifecycleCollectionPersistence{
	...
	result, err := service.CollectLifecycle(SystemActor(), request)
	if err != nil || len(result.DeletedRunIDs) != 1 {
		t.Fatalf("visible transaction retry result=%+v err=%v", result, err)
	}
}
...
func TestLifecycleCollectionCompletedHeaderUncertaintyBlocksPeerUntilMatchingRetry(t *testing.T) {
	...
	actor := SystemActor()
	dryRun, err := service.CollectLifecycle(actor, CollectionRequest{})
	if err != nil || len(dryRun.Plan.Candidates) != 0 {
		t.Fatalf("build empty collection plan: candidates=%d err=%v", len(dryRun.Plan.Candidates), err)
	}
	service.store.collectionPersistence = &faultingLifecycleCollectionPersistence{
	...
	if !errors.Is(err, ErrConflict) {
		t.Fatalf("peer accepted uncertain completed receipt: %v", err)
	}

	result, err := service.CollectLifecycle(actor, request)
	if err != nil || !result.Applied || len(result.Plan.Candidates) != 0 {
		t.Fatalf("matching completed receipt retry=%+v err=%v", result, err)
	}
	if service.store.lifecycle.pendingCollection != nil {
...
func TestLifecycleCollectionRetryRepairsPartialProgressTail(t *testing.T) {
	...
	data, err := os.ReadFile(service.store.lifecycleCollectionPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	result, err := service.CollectLifecycle(actor, request)
	if err != nil || len(result.DeletedRunIDs) != 1 {
		t.Fatalf("repair partial progress tail: result=%+v err=%v", result, err)
	}
	data, err = os.ReadFile(service.store.lifecycleCollectionPath())
	if err != nil || !bytes.HasSuffix(data, []byte{'\n'}) {
		t.Fatalf("repaired collection log boundary: err=%v", err)
	}
}
