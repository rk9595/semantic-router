...
func (s *Store) createCampaignAsUnlocked(actor Actor, campaign Campaign) error {
	...
	campaignBytes, err := campaignJSONSize(campaign)
	if err != nil {
		return err
	}
	lifecycleBytes, err := campaignLifecycleAdmissionBytes(lifecycle)
	if err != nil {
		return err
	}
	bundleBytes, err := checkedLifecycleBytes(campaignBytes, lifecycleBytes)
	if err != nil {
		return err
	}
	...
	staged, err := os.MkdirTemp(s.campaignRoot, stagedCampaignPrefix)
	if err != nil {
...
func (s *Store) deleteCampaignAsUnlocked(actor Actor, id string, collection bool) error {
	...
	campaign, lifecycle, err := s.campaignForActorUnlocked(actor, id)
	if err != nil {
		if validClientRequestID(id) {
			...
			if _, auditErr := s.appendLifecycleAuditUnlocked(
				actor, lifecycleResourceCampaign, "delete", "denied", lifecycleDenialReason(err), id, owner,
			); auditErr != nil {
				return auditErr
			}
		}
