...
func TestServiceCloseCancelsAndWaitsForWorkerExit(t *testing.T) {
	...
	pending, pendingErr := service.CreateRunAs(context.Background(), SystemActor(), validCreateRequest())
	if pendingErr != nil {
		t.Fatalf("CreateRun after Close: %v", pendingErr)
	}
	if _, err := service.StartRunAs(context.Background(), SystemActor(), pending.ID); !errors.Is(err, ErrConflict) {
		t.Fatalf("StartRun after Close error=%v, want ErrConflict", err)
	}
	if _, _, err := service.SubscribeAs(SystemActor(), pending.ID); !errors.Is(err, ErrConflict) {
