...
func TestCandidateBaselineAuthorizationHidesForeignState(t *testing.T) {
	...
	completed, err := service.CreateRunAs(context.Background(), owner, validCreateRequest())
	if err != nil {
		t.Fatalf("create completed foreign baseline: %v", err)
	}
	completed = completeTestRun(t, service, completed)
	pending, err := service.CreateRunAs(context.Background(), owner, validCreateRequest())
	if err != nil {
		t.Fatalf("create pending foreign baseline: %v", err)
	}
	incomparableRequest := validCreateRequest()
	incomparableRequest.Seed++
	incomparable, err := service.CreateRunAs(context.Background(), owner, incomparableRequest)
	if err != nil {
		t.Fatalf("create incomparable foreign baseline: %v", err)
	}
	...
	_, err = service.CreateRunAs(context.Background(), other, request)
	if !errors.Is(err, ErrForbidden) || err.Error() != wantMessage {
		t.Fatalf("unreadable foreign baseline leaked state: error=%q want=%q", err, wantMessage)
	}
