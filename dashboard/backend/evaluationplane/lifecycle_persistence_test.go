...
func TestMatchingLifecycleRetryRecoversWhenFailedWriteCouldNotReadCanonicalValue(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("create lifecycle retry run: %v", err)
	}
	...
	view, err := service.UpdateRunLifecycle(owner, run.ID, request)
	if err != nil || !view.EvidenceHold {
		t.Fatalf("matching retry did not reconcile unpublished lifecycle write: view=%+v err=%v", view, err)
	}
...
func TestRunLifecycleRetryClosesVisiblePolicySyncFailure(t *testing.T) {
	...
	run, err := service.CreateRunAs(context.Background(), owner, validCreateRequest())
	if err != nil {
		t.Fatalf("create lifecycle durability run: %v", err)
	}
	...
	if err != nil || !view.EvidenceHold || view.RetentionClass != RetentionProtected || view.DeleteAfter != nil {
		t.Fatalf("run lifecycle retry did not commit protected hold: view=%+v err=%v", view, err)
	}
	...
	if err != nil || len(plan.Plan.Candidates) != 0 || plan.Plan.Skipped["held"] != 1 {
		t.Fatalf("protected held run entered collection before restart: plan=%+v err=%v", plan.Plan, err)
	}
	...
	view, err = restarted.RunLifecycle(owner, run.ID)
	if err != nil || !view.EvidenceHold || view.RetentionClass != RetentionProtected || view.DeleteAfter != nil {
		t.Fatalf("protected hold changed after restart: view=%+v err=%v", view, err)
	}
	plan, err = restarted.CollectLifecycle(SystemActor(), CollectionRequest{})
	if err != nil || len(plan.Plan.Candidates) != 0 || plan.Plan.Skipped["held"] != 1 {
		t.Fatalf("protected held run entered collection after restart: plan=%+v err=%v", plan.Plan, err)
	}
...
func TestRunLifecycleReleaseCannotBypassPendingCommitDuringCollection(t *testing.T) {
	...
	run, err := service.CreateRunAs(context.Background(), owner, validCreateRequest())
	if err != nil {
		t.Fatalf("create lifecycle release run: %v", err)
	}
	...
	faults := &faultingLifecyclePersistence{
		delegate: atomicLifecyclePolicyPersistence{}, writeFailures: 1, syncFailures: 2,
	}
	service.store.lifecyclePersistence = faults
	...
	view, err := service.UpdateRunLifecycle(owner, run.ID, release)
	if err != nil || view.EvidenceHold || view.RetentionClass != RetentionEphemeral {
		t.Fatalf("run release retry did not close commit: view=%+v err=%v", view, err)
	}
	plan, err := service.CollectLifecycle(SystemActor(), CollectionRequest{})
	if err != nil || len(plan.Plan.Candidates) != 1 || plan.Plan.Candidates[0].RunID != run.ID {
		t.Fatalf("durable run release did not become collectable: plan=%+v err=%v", plan.Plan, err)
	}
	...
	plan, err = restarted.CollectLifecycle(SystemActor(), CollectionRequest{})
	if err != nil || len(plan.Plan.Candidates) != 1 || plan.Plan.Candidates[0].RunID != run.ID {
		t.Fatalf("run release changed after restart: plan=%+v err=%v", plan.Plan, err)
	}
}
