...
func TestCampaignProtocolIsPresentOnlyOnDeclaredSuitesAndRejectsLegacyFields(t *testing.T) {
	...
	campaignJSON, err := json.Marshal(suites[1])
	if err != nil {
		t.Fatal(err)
	}
	...
	nonCampaignJSON, err := json.Marshal(suites[0])
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Contains(nonCampaignJSON, []byte(`"campaign_protocol"`)) {
		t.Fatalf("non-campaign suite emitted a campaign protocol: %s", nonCampaignJSON)
	}
